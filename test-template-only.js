  ...
  static async processIncludes(template) {
    ...
    for (const match of matches) {
      ...
      try {
        ...
        const fullPath = path.join(__dirname, "templates", includePath);
...
const testData = {
  ...
  description:
    "Đây là test article để kiểm tra header và footer có hiển thị đúng không",
  content:
    "<h2>Nội dung test</h2><p>Đây là nội dung test để xem template có render đúng header và footer không.</p>",
...
async function testTemplate() {
  try {
    ...
    const result = await SimpleTemplateEngine.replaceVariables(
      template,
      testData
    );
